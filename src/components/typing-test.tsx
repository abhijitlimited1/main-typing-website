...
const TypingTest = ({ duration }: TypingTestProps) => {
  const [difficulty, setDifficulty] = useState<Difficulty>('medium');
  const [timeOption, setTimeOption] = useState<number>(duration || 60); // Use provided duration or default to 60 seconds
  ...
  // Set initial timeOption based on duration prop
  useEffect(() => {
    if (duration) {
 ...
 // Timer logic
  ...
  const renderText = () => {
    ...
    const renderedChars = characters.map((char, index) => {
      ...
      let className = 'text-gray-700 dark:text-gray-300'; // Explicit colors for better visibility
   ...
   const handleDownloadPdf = () => {
    ...
    doc.setFontSize(10);
    doc.text("Generated by TypeAce", 105, 140, { align: 'center' });
  ...
  return (
    <Card className="w-full max-w-3xl shadow-lg">
      <CardHeader>
        <CardTitle className="text-3xl font-bold text-center text-primary dark:text-primary">TypeAce ⌨️</CardTitle>
        <CardDescription className="text-center text-gray-700 dark:text-gray-300">Test your typing speed and accuracy!</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-6">
           ...
           <Dialog>
            ...
            <DialogContent className="w-[90vw] max-w-[425px] bg-white dark:bg-gray-800 text-gray-900 dark:text-white border-primary/20">
              <DialogHeader>
                <DialogTitle className="text-xl text-primary dark:text-primary">Settings</DialogTitle>
              ...
              <div className="grid gap-6 py-4">
                 <div className="grid grid-cols-1 sm:grid-cols-4 items-center gap-2 sm:gap-4">
                    <Label htmlFor="time-select" className="text-left sm:text-right text-gray-800 dark:text-gray-200 font-medium mb-1 sm:mb-0">
                      Time
                    </Label>
                     <Select
                        ...
                        name="time-select"
                      >
                        <SelectTrigger className="col-span-1 sm:col-span-3 bg-background dark:bg-gray-800 text-foreground dark:text-white border-primary/30 hover:border-primary/50 focus:border-primary h-10 px-3 py-2">
                          <SelectValue placeholder="Select time" />
                        </SelectTrigger>
                        <SelectContent className="bg-background dark:bg-gray-800 text-foreground dark:text-white border-primary/30 min-w-[180px]">
                           <SelectItem value="30" className="hover:bg-primary/10 focus:bg-primary/20 cursor-pointer text-foreground dark:text-white">30 seconds</SelectItem>
                           <SelectItem value="60" className="hover:bg-primary/10 focus:bg-primary/20 cursor-pointer text-foreground dark:text-white">1 minute</SelectItem>
                           <SelectItem value="180" className="hover:bg-primary/10 focus:bg-primary/20 cursor-pointer text-foreground dark:text-white">3 minutes</SelectItem>
                           <SelectItem value="300" className="hover:bg-primary/10 focus:bg-primary/20 cursor-pointer text-foreground dark:text-white">5 minutes</SelectItem>
                        </SelectContent>
                      </Select>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-4 items-center gap-2 sm:gap-4">
                    <Label htmlFor="difficulty-select" className="text-left sm:text-right text-gray-800 dark:text-gray-200 font-medium mb-1 sm:mb-0">
                      Difficulty
                    </Label>
                    <div className="col-span-1 sm:col-span-3">
                      <div className="grid grid-cols-3 gap-2">
                        <Button 
                          ...
                          className={`py-2 px-3 rounded-md transition-all ${difficulty === 'easy' 
                            ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-100 border-2 border-green-300 dark:border-green-700 font-medium' 
                            : 'bg-card text-foreground border border-gray-300 dark:border-gray-600 hover:bg-green-50 dark:hover:bg-green-900/30'}`}
                          disabled={status === 'running'}
                        >
                          Easy
                        </Button>
                        <Button 
                          ...
                          className={`py-2 px-3 rounded-md transition-all ${difficulty === 'medium' 
                            ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-100 border-2 border-blue-300 dark:border-blue-700 font-medium' 
                            : 'bg-card text-foreground border border-gray-300 dark:border-gray-600 hover:bg-blue-50 dark:hover:bg-blue-900/30'}`}
                          disabled={status === 'running'}
                        >
                          Medium
                        </Button>
                        <Button 
                          ...
                          className={`py-2 px-3 rounded-md transition-all ${difficulty === 'hard' 
                            ? 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-100 border-2 border-red-300 dark:border-red-700 font-medium' 
                            : 'bg-card text-foreground border border-gray-300 dark:border-gray-600 hover:bg-red-50 dark:hover:bg-red-900/30'}`}
                          disabled={status === 'running'}
                        >
                          Hard
                        </Button>
                      ...
                      <div className="mt-2 text-xs text-gray-600 dark:text-gray-300">
                        ...
                        {difficulty === 'medium' && 'Moderate vocabulary and average sentence length'}
                        {difficulty === 'hard' && 'Complex words and longer sentences for experts'}
                      </div>
                    </div>
                  </div>
              </div>
          ...
          <div className="flex items-center justify-center gap-2 p-2 rounded-md bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-white font-mono text-xl min-w-[80px]">
        ...
        {/* Text display area - Render conditionally based on status */}
         {status !== 'finished' ? (
            <div
              className="relative leading-relaxed tracking-wide p-6 border-2 border-primary/30 rounded-lg bg-white dark:bg-gray-800 text-foreground dark:text-white cursor-text min-h-[180px] overflow-auto shadow-md"
            ...
            >
              ...
              {status === 'idle' && !userInput && text && ( // Show placeholder only when idle, no input yet, and text is loaded
                <div className="absolute inset-0 flex flex-col items-center justify-center text-primary dark:text-primary font-medium pointer-events-none bg-primary/5 dark:bg-primary/10 rounded-md">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="animate-pulse text-2xl">⌨️</span>
                    <span className="text-xl md:text-2xl text-foreground dark:text-foreground">Ready to test your typing speed?</span>
                  </div>
                  <div className="bg-primary/10 dark:bg-primary/20 px-4 py-2 rounded-full mb-2">
                    <p className="text-sm md:text-base font-medium text-foreground dark:text-foreground">Click here or start typing to begin</p>
                  </div>
                  <p className="text-sm text-foreground dark:text-foreground opacity-70">The timer will start automatically when you begin typing</p>
                </div>
              )}
        ...
        {status === 'running' && (
          <div className="flex justify-around items-center p-2 bg-gray-100 dark:bg-gray-700 rounded-md text-gray-800 dark:text-white animate-in fade-in duration-300">
            <div className="text-center">
             ...
             <div className="text-center">
              ...
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{correctChars}</p>
            </div>
             <div className="text-center">
              <p className="text-sm font-semibold">Incorrect</p>
              <p className="text-2xl font-bold text-red-600 dark:text-red-400">{incorrectChars}</p>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="mt-6 bg-blue-50 border border-blue-100 rounded-lg p-4 text-center">
              <p className="text-blue-800 text-sm">
                <span className="font-medium">Pro Tip:</span> Take regular tests to track your progress over time. 
                Consistent practice is the key to improving your typing speed!
              </p>
            </div>
          </>
        )}
       ...
       <CardFooter className="flex justify-center pt-4 gap-4">
         ...
         {status === 'finished' && (
            <>
               ...
               <EnhancedButton onClick={() => {
                    ...
                    const resultText = `I achieved ${isNaN(wpm) ? 0 : wpm} WPM with ${isNaN(accuracy) ? 0 : accuracy}% accuracy on TypeAce! (${correctChars}✓ / ${incorrectChars}✗ in ${timeOption}s - ${difficulty})`;
                    if (navigator.share) {
                      navigator.share({
         ...
         {status === 'idle' && (
              <Button 
                ...
                variant="default" 
                size="lg"
                className="bg-primary text-white dark:text-white font-medium"
              >
                Start Typing
              </Button>
            )}
